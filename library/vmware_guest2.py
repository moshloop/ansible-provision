...
class PyVmomiHelper(PyVmomi):
    ...
    def configure_network(self, vm_obj):
        ...
        for key in range(0, len(network_devices)):
            ...
            if nic_change_detected:
                self.networks.append(nic.device.deviceInfo.summary)
    ...
    def deploy_vm(self):
        ...
        if len(self.params['customization']) > 0 or customize_network:
        ...
        if task.info.state == 'error':
            ...
            if self.params['wait_for_ip_address'] or self.params['state'] in ['poweredon', 'restarted']:
                ...
                if self.params['wait_for_ip_address']:
                    self.wait_for_vm_ip(vm, self.params.get('wait_for_ip_address_timeout', 100))

            vm_facts = self.gather_facts(vm)
            results = {'changed': self.change_detected, 'failed': False, 'instance': vm_facts}
            if 'debug' in self.params and self.params['debug']:
                results['clonespec'] = serialize_spec(clonespec)
                results['configspec'] = serialize_spec(self.configspec)
                results['networks'] = self.networks
            return results
...
def main():
    ...
    argument_spec.update(
        ...
        wait_for_ip_address_timeout=dict(type='int', default=100),
